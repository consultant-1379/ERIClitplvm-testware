...
public class LITPlvmTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPlvmTestRunner.class);
    ...
    @TestId(id = "CXP9031118-1", title = "Upgrade ERIClitplvm rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitplvmRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("lvm");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031118-2", title = "Run python test cases for ERIClitplvm package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitplvmTests() {
     ...
     * @PRE Execution of test {@link #runERIClitplvmTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031118-3", title = "Parse xml outputs from python tests")
